...
export default function DriverMasterScreen() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      console.error(error);
      Alert.alert('Error', 'Gagal memuat data driver');
    } finally {
      setLoading(false);
    }
  };
      ...
      [
        ...
        { 
          ...
          onPress: async () => {
            try {
              const db = await getDbClient();
              await db.query('DELETE FROM drivers WHERE id = $1', [item.id]);
              await db.end();
              loadData();
            } catch (error) {
              console.error(error);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingItem) {
        await db.query(
          'UPDATE drivers SET name = $1, status = $2 WHERE id = $3',
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Master Driver</Text>
          ...
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>
                {editingItem ? 'Edit Driver' : 'Tambah Driver'}
              </Text>
