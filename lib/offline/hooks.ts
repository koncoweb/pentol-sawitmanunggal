...
export const useOfflineData = () => {
  ...
  const getDivisi = async () => {
    ...
    if (netInfo.isConnected) {
      try {
        ...
        const { rows } = await client.query('SELECT * FROM divisi ORDER BY name');
        await client.end();
        return rows as OfflineDivisi[];
      } catch (e) {
        console.error('Online fetch failed, falling back to local:', e);
      }
    }
    // Fallback to local
  ...
  const getGang = async (divisiId: string) => {
     ...
     if (netInfo.isConnected) {
         try {
             const client = await getDbClient();
             const { rows } = await client.query('SELECT * FROM gang WHERE divisi_id = $1 ORDER BY name', [divisiId]);
             await client.end();
  ...
  const getBlok = async (divisiId: string) => {
      ...
      if (netInfo.isConnected) {
          try {
              const client = await getDbClient();
              const { rows } = await client.query('SELECT * FROM blok WHERE divisi_id = $1 ORDER BY name', [divisiId]);
              await client.end();
      ...
      if (netInfo.isConnected) {
          try {
              ...
              const { rows } = await client.query(`
                  SELECT p.id, g.divisi_id, p.gang_id, COALESCE(p.nik, '') as operator_code, p.name, 1 as active
                  FROM pemanen p
